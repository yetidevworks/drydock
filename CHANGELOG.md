...
1. [](#new)
    ...
    * **Adding an org is gated on tool auth and infers what the tooling knows.** `drydock org add` reads the providers and hosts `gh`, `glab` and `tea` are actually authenticated to — one logged-in CLI means provider and host need no flags, a self-hosted instance is only ever named from that tool's own logins, and a provider without a logged-in CLI is refused with the login command that would unlock it. The dashboard's add form works the same way, with the owner picked from a list fetched for the chosen host. Registration also wires the checkouts into discovery: when the org's path isn't already under a scan root, its parent is added to `roots` automatically (at `org add`, and on sync for hand-edited configs), so cloned repos land on the dashboard as a group with no manual config edit — the old opt-in `--root` flag is gone because it is now just what happens.
